...
export default function TopBar({ title, onMobileMenuToggle, showMobileMenu }: TopBarProps) {
  ...
  // Use single BTC ticker for top bar
  const mainTickers = btcPrice ? [btcPrice] : [];
    ...
    <header className="bg-card border-b border-border sticky top-0 z-40">
      <div className="flex items-center justify-between p-4 lg:p-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          {/* Page Title */}
          <div>
            <h1 className="text-xl lg:text-2xl font-bold text-slate-900 dark:text-slate-100">{title}</h1>
            {user?.role === 'admin' && (
            ...
            <div key={ticker.symbol} className="bg-muted px-4 py-2 rounded-lg min-w-[140px]">
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <span className={`text-xs ${
                  (ticker.changePercent24h ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'
                }`}>
