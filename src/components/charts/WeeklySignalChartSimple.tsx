  ...
  const loadChartData = async () => {
    try {
      ...
      const [ohlcResponse, signalsResponse, priceResponse] = await Promise.all([
        fetch(`/api/public/ohlc?symbol=${ticker}&interval=1w&limit=104`).then(r => r.json()),
        fetch(`/api/public/signals/alerts?ticker=${ticker}&timeframe=1W`).then(r => r.json()),
        fetch(`/api/public/market/price/${ticker}`).then(r => r.json())
      ]);
