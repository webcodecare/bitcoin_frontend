...
export default function WeeklySignalsStandalone({ 
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // Only show "connecting" status on initial load or after an error
        if (isInitialLoad || connectionStatus === 'error') {
        ...
        if (ohlcResponse?.data?.length && candlestickSeriesRef.current) {
          ...
          // Add signal markers - simplified without text
          if (signalsResponse?.length) {
            const markers = signalsResponse.map((signal: AlertSignal) => ({
              ...
              size: 1, // Smaller, cleaner markers
            }));
