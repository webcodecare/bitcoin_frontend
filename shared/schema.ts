...
export const users = pgTable("users", {
  ...
  subscriptionTier: text("subscription_tier", { enum: ["free", "basic", "premium", "pro", "elite"] }).notNull().default("free"),
...
// User Ticker Subscriptions (renamed to avoid conflict with payment subscriptions)
...
export const userAlerts = pgTable("user_alerts", {
  ...
  ticker: text("ticker").notNull(),
  condition: text("condition", { enum: ["above", "below", "crosses_above", "crosses_below", "change_up", "change_down", "volume_spike", "rsi_overbought", "rsi_oversold"] }).notNull(),
...
export const insertUserTickerSubscriptionSchema = createInsertSchema(userTickerSubscriptions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type UserTickerSubscription = typeof userTickerSubscriptions.$inferSelect;
export type InsertUserTickerSubscription = z.infer<typeof insertUserTickerSubscriptionSchema>;
...
export const subscriptionPlans = pgTable("subscription_plans", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: text("name").notNull(),
  tier: text("tier", { enum: ["free", "basic", "premium", "pro", "elite"] }).notNull().unique(),
...
export const userSubscriptions = pgTable("user_subscriptions", {
  ...
  status: text("status", { 
    enum: ["active", "cancelled", "past_due", "trialing", "incomplete", "paused"] 
